...
public class HashExtensivel<T extends RegistroHashExtensivel<T>> {
  ...
  /**
   * Lista todos os elementos armazenados na hash extensível.
   * Útil para buscar múltiplos elementos que atendam a um critério.
   * @return ArrayList com todos os elementos
   */
  public ArrayList<T> listAll() throws Exception {
    ArrayList<T> todosElementos = new ArrayList<>();
    ...
    while (arqCestos.getFilePointer() < arqCestos.length()) {
      ...
      // Adicionar todos os elementos não vazios do cesto
      for (int i = 0; i < c.quantidade; i++) {
        todosElementos.add(c.elementos.get(i));
      }
    }
